...
def plot_tracks(day, obs_ids, usecols, save=True):
    ...
    selection['ID'] = selection['ID'] % 10000 #discard the date and time info, keep only the raw ID
    ...
    for i in range(len(mod_tracks)):
        ax1.plot(mod_tracks[i]['lon'], mod_tracks[i]['lat'], 'x--', linewidth=1, markersize=2, label=str(mod_tracks[i]['cell_id']), transform=ccrs.PlateCarree())
    ax1.legend(loc='upper left', fontsize='5')
    ax1.title.set_text("modelled supercells")

    ax2 = fig.add_subplot(2, 1, 2, projection=ccrs.PlateCarree(), sharex=ax1)
...
plot_tracks(day, obs_ids, usecols)#, False)
